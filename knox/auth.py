...
class TokenAuthentication(BaseAuthentication):
    ...
    def authenticate_credentials(self, token):
        ...
        for auth_token in AuthToken.objects.filter(
            is_active=True, token_key=token[:CONSTANTS.TOKEN_KEY_LENGTH]
        ):
    ...
    def _cleanup_token(self, auth_token):
        # Take all user tokens except auth_token and deactivate them if needed
        for other_token in auth_token.user.auth_token_set.filter(
            is_active=True
        ).exclude(pk=auth_token.pk):
            ...
            if other_token.digest != auth_token.digest and other_token.expiry:
                if other_token.expiry < timezone.now():
                    ...
                    other_token.save(update_fields=['is_active'])
                    token_expired.send(
